...
def upgrade() -> None:
    ...
    op.add_column(
        "users",
        sa.Column("preferred_portion_size", sa.String(length=50), nullable=True),
    )
    op.add_column(
        "users", sa.Column("max_prep_time_minutes", sa.Integer(), nullable=True)
    )
    op.add_column(
        "users", sa.Column("max_cook_time_minutes", sa.Integer(), nullable=True)
    )
